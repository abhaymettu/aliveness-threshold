...
class Voice:
    ...
    def synth(self, text: str, trim: bool = True) -> np.ndarray:
        ...
        x = self._piper_synth(text) if self.backend == "piper" else self._say_synth(text)
    ...
    def _piper_synth(self, text: str) -> np.ndarray:
        chunks = []
        sr = audio.SR
    ...
    def _say_synth(self, text: str) -> np.ndarray:
